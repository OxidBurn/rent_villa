          ...
          {destinationColumns.map((column, columnIndex) => (
            <div key={columnIndex} className="flex flex-col gap-8 md:gap-12 flex-1">
              {column.map((destination, itemIndex) => (
                <ScrollReveal key={destination.id} delay={columnIndex * 100 + itemIndex * 150} direction={columnIndex % 2 === 0 ? 'up' : 'down'}>
                  ...
                  <div className={`relative w-full ${destination.aspectRatio} rounded-xl overflow-hidden shadow-lg group-hover/travel:shadow-2xl transition-shadow duration-300`}>
                    <Image
                    ...
                    <button
                      onClick={() => console.log('Travel destination clicked:', destination.id)}
