...
const popularVillas: Villa[] = [
  ...
  },
  {
    ...
    description: 'Роскошный "водный дворец" на вершине хребта, откуда открываются захватывающие бирюзовые виды на Атлантический океан.',
    image: '/image 2.png',
    category: 'beachfront',
    maxGuests: 24,
    bedrooms: 10,
  ...
  },
  {
    ...
    location: 'Чангу, Бали',
    description: 'Это божественная резиденция современного дизайна, раскинувшаяся на побережье в окружении тропического великолепия, где утонченная роскошь сочетается с беззаботным ритмом океанских волн.',
    image: '/image 3.png',
    category: 'premium',
    maxGuests: 12,
    bedrooms: 6,
...
export default function PopularVillas() {
  ...
  const handlePrevious = () => {
    setCurrentIndex((prev) => (prev > 0 ? prev - 1 : popularVillas.length - 1));
  };

  const handleNext = () => {
    setCurrentIndex((prev) => (prev < popularVillas.length - 1 ? prev + 1 : 0));
  };
          ...
          <div className="flex gap-5 px-4 md:px-8 lg:px-12 pb-4">
            {popularVillas.map((villa) => (
              <div key={villa.id} className="flex-shrink-0 w-5/6 sm:w-3/5 md:w-1/2 lg:w-2/5">
                <VillaCard
                  villa={villa}
                  onClick={() => console.log('Villa clicked:', villa.id)}
                />
              </div>
            ))}
          </div>
