  ...
  return (
    <div className="flex flex-col gap-4 w-full group/villa cursor-pointer transition-transform duration-300 hover:-translate-y-2">
      <div className="flex flex-col gap-2 relative">
        <div className={`${config.bgColor} ${config.textColor} flex items-center gap-2 px-3 py-1 rounded-full w-fit uppercase text-sm md:text-base transition-transform duration-300 group-hover/villa:scale-105`}>
          <Image src={config.icon} alt="" width={16} height={16} />
          <span>{config.label}</span>
        </div>

        <div className={`relative w-full ${villa.aspectRatio || 'aspect-[4/3]'} rounded-xl overflow-hidden shadow-lg group-hover/villa:shadow-2xl transition-shadow duration-300`}>
          <Image
            src={villa.image}
            alt={villa.title}
            fill
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-4 md:gap-6">
          <IconText
            icon="/user 1.svg"
            text=" Гостей"
            highlighted={`До ${villa.maxGuests}`}
          />
          <IconText
            icon="/bed-alt 1.svg"
            text=" Спальни"
            highlighted={`${villa.bedrooms}`}
          />
          <IconText
            icon="/bath 1.svg"
            text=" Ванных комнат"
            highlighted={`${villa.bathrooms}`}
          />
        </div>
      </div>
    </div>
